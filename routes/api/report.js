...
router.get('/printMe', function(req, res, next) { //, Middleware.isAuthenticated
   Report.jobReport(req, res, function(err, dat) {
      if (err) {
         console.log(err);
...
router.post('/processReport', Middleware.isAuthenticated, function(req, res, next) {
   Report.processReport(req, res, function(err, dat) {
      if (err) {
         console.log(err);
         return res.status(500).send(err);
      }
